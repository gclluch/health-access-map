...
| **Acceptability** - ACS Medicaid-coverage rate (this session) | +0.278 → partial **−0.064** | The non-spatial demand-side proxy. Strong raw signal but **collapses in partial-r** (corr +0.57 socioeconomic, +0.59 health_need) - it is the poverty/deprivation gradient already scored, restated. Supply-side acceptance (do providers take Medicaid) has **no free national data** (C4); only the scrape-to-calibrate build can deliver it. Not scored (redundant, not a new axis - fails the broadband/completeness bar too). |
